...
,Published,P13FL9,Planet 13 Neighborhood Dispensary Edgewater,1308 S Ridgewood Ave,Edgewater,FL,US,32132,(386) 287-5539,09:00-19:00,10:00-18:00,2025-05-21
,Published,P13FL8,Planet 13 Neighborhood Dispensary Daytona,1027 N Nova Rd,Holly Hill,FL,US,32117,(386) 555-0100,09:00-19:00,10:00-18:00,2024-11-01
,Published,P13FL99,Planet 13 Somewhere New,1 New St,Nowhere,FL,US,00000,,09:00-19:00,,2026-01-01
...
def test_import_matches_by_zip_and_locality(session):
    session.add_all([
        Store(code="HS10135", name="FL - Planet 13 - Edgewater", address="FL 32132", state="FL", latitude=29.0, longitude=-81.0),
        Store(code="HS10108", name="FL - Planet 13 - Daytona", address="FL 32117", state="FL"),
        Store(code="HS2328", name="NV - Planet 13 - Vegas (REC)", address="NV 89109", state="NV"),
    ])
    session.commit()
    ...
    edge = session.execute(select(Store).where(Store.code == "HS10135")).scalar_one()
    ...
    daytona = session.execute(select(Store).where(Store.code == "HS10108")).scalar_one()
...
def test_match_store_prefers_zip_then_locality():
    a = Store(code="A", name="FL - Planet 13 - Tampa Kennedy", address="x, Tampa, FL 33606", state="FL")
    b = Store(code="B", name="FL - Planet 13 - Tampa Bruce B Downs", address="y, Tampa, FL 33647", state="FL")
